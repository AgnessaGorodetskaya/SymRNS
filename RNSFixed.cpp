...
class RnsBase {
   public:
    ...
    // std::vector<Positional_Int> C;  // остаток от Bi по S
    // Positional_Int Ms;  // остаток P по S
...
Positional_Int RnsNumber::get_remainder(Positional_Int divisor) const {
    Positional_Int res = 0;
    for (size_t i = 0; i < a.size(); ++i) {
        res += RnsNumber::mod(base.get().Pi[i] * RnsNumber::mod(a[i] * base.get().m[i], base.get().p[i]), divisor);
    }
    Positional_Int rank = get_rank();
    res -= RnsNumber::mod(rank * base.get().P, divisor);
    res = RnsNumber::mod(res, divisor);
    return res;
}
...
void RnsNumber::div_int(const RnsNumber& y) {  // деление нацело
    for (size_t i = 0; i < a.size(); ++i) {
