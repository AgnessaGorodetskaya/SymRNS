...
int main() {
    ...
    // for (int x= -10; x <= 10; ++x) {
    //     std::cout << "x=" << x << " x^-1=" << RnsFixed::mod_inverse(x, 5) << std::endl;
    // }
    ...
    for (Positional_Int x_pos = 0; x_pos < base.P; ++x_pos) {
        RnsFixed x_rns = RnsFixed{x_pos, base};
    ...
    //     RnsFixed x_rns = RnsFixed{x, base};
    ...
    //         RnsFixed r_rns = -RnsFixed(x_int, base, q);
    //         RnsFixed res_rns = RnsFixed(res_int, base, q);
    //         if (res_rns != r_rns) {
    ...
    //         res_int = x_int + y_int;
    //         if (res_int >= -base.P/2 && res_int <= base.P/2) {
    //             RnsFixed r_rns = RnsFixed(x_int, base, q) + RnsFixed(y_int, base,
    //             q); RnsFixed res_rns = RnsFixed(res, base, q); if (res_rns != r_rns) {
    ...
    //         base.P/2) {
    //             RnsFixed r_rns = RnsFixed(x_int, base, q) * RnsFixed(y_int, base,
    //             q); RnsFixed res_rns = RnsFixed(res, base, q*q); if (res_rns != r_rns) {
    //                 std::cout << "ОШИБКА: x: " << static_cast<Positional_Float>(x_int) / q << " *
    //                 y:"
    //                     << static_cast<Positional_Float>(y_int) / q << " convert: " <<
    //                     res_rns.to_positional()
    ...
    RnsFixed a{a_int, base};
    RnsFixed b{b_int, base};
    RnsFixed c = a * b;
    ...
    a = RnsFixed{a_int, base};
    b = RnsFixed{b_int, base};
    cc_fl = a_fl / b_fl;
    c = a / b;
    ...
    std::vector<RnsFixed> v(VSIZE, RnsFixed{0, base});
    for (size_t i = 0; i < VSIZE; ++i) {
        v[i] = RnsFixed(std::rand() % base.P, base);
    }
