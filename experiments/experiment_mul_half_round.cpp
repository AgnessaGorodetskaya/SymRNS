...
int main(void) {
    ...
    constexpr Positional_Int a_int_orig = 10;
    ...
    for (Positional_Int z_int = 2; z_int <= 100; ++z_int) {
        Positional_Int b_int_raw = z_int * S + S / 2;
        if (b_int_raw % a_int_orig == 0) {
            Positional_Int b_int = b_int_raw / a_int_orig;
            ...
            res_srns += mul_srns;
            std::cout << "ССОК Фикс.точка: " << std::setprecision(100) << mul_srns.to_positional_frac_crt_unscaled() << std::endl;

            RnsFixed mul_rns = a_rns * b_rns;
            res_rns += mul_rns;
            std::cout << " СОК Фикс.точка: " << std::setprecision(100) << mul_rns.to_positional_frac_crt_unscaled() << std::endl << std::endl;
        } else {
