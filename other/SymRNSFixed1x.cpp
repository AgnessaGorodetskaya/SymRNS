        ...
        S{S0}
{
    ...
    for (size_t i = 0; i < p.size(); ++i) {
        ...
        std::cout << "P["<<i+1<<"]="<<Pi[i]<<" B[" << i+1 << "]=" << B[i] << ' ' << B_rns << ", вес базиса m" << i+1 << "=" << m[i] << std::endl;
    }
...
Positional_Float SymRnsFixed::frac_crt_sum() const {
    ...
    for (size_t i = 0; i < a.size(); ++i) {
        Positional_Int aimi = a[i] * base.get().m[i];
        Module ms = mod_sym(aimi, base.get().p[i]);
        std::cout << "Ai*Mi=" << aimi << " MS" << i+1 << "= " << ms << " / " << base.get().p[i] << " = " << std::endl;
        pos_float += static_cast<Positional_Float>(ms) / base.get().p[i];
    }
    return pos_float;
}
...
Positional_Int SymRnsFixed::get_remainder(Positional_Int divisor) const {
    ...
    for (size_t i = 0; i < a.size(); ++i) {
        Module ms = mod_sym(a[i] * base.get().m[i], base.get().p[i]);
        rem_int += mod_sym(base.get().Pi[i] * ms, divisor);
    }
...
void SymRnsFixed::div_int(const SymRnsFixed& y) {  // деление нацело (когда точно делится)
    std::cout << "DIV_INT(y): " << y << y.to_positional_frac_crt_unscaled() << std::endl;
    for (size_t i = 0; i < a.size(); ++i) {
        Positional_Int Si = mod_inverse_sym(y.a[i], base.get().p[i]);
        std::cout << "S" << i+1 << "=" << Si << std::endl;
...
SymRnsFixed& SymRnsFixed::operator/=(const SymRnsFixed& y) {
    ...
    SymRnsFixed remainder_rns{remainder, base};
    std::cout << " REM=" << remainder_rns << remainder;

    *this -= remainder_rns;
    std::cout << " WO_REM=" << *this << to_positional_frac_crt_unscaled();
    div_int(y);
...
bool SymRnsFixed::operator<(const SymRnsFixed &y) const
{
    ...
    std::cout << std::fixed << std::setprecision(20) << "x_fl=" << x_fl << " y_fl=" << y_fl << std::endl;
